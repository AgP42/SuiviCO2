...
class suiviCO2 extends eqLogic
{
    ...
    {
        //  fb 8/5/20 ajout du parametre $_datetime_date pour forcer la date et l heure de la mesure dans cmd->event
        ...
        if ($consumption < 15 * 1000) { //13 kWh pour compteur 63 A  par heure c'est qu'on a un gros probleme... Ce test permet de ne pas sauvegarder l'index entier lors de la 1ere boucle apres la creation de l'objet.
            $cmd = $this->getCmd(null, 'consumption' . $_type);
            if (is_object($cmd)) {
                $cmd->setCollectDate($_datetime_data);
                log::add('suiviCO2', 'debug', 'eqLogic_id : ' . $this->getId() . ' - Index now ' . $_type . ' : ' . $index . ' - Prev Index ' . $_type . ' : ' . $lastValue . ' = conso ' . $_type . ' (Wh) : ' . $consumption . ' date de event :' . $_datetime_data);
    ...
    public function calculTotauxCo2($_datetime_data)
    {
        // appellé par le cron Hourly, pour les calculs de totaux sur le dashboard
        //  fb 8/5/20 ajout du parametre $_datetime_date pour forcer la date et l heure de la mesure dans cmd->event
        ...
        if (is_object($cmdTotalDay) && $cmdTotalDay->getIsVisible()) { // si cette commande existe et qu'elle est visible
            ...
            $cmdTotalDay->setCollectDate($_datetime_data);
        ...
        if ($cmdTotalWeek->getIsVisible()) {
            ...
            $totalCO2 = $this->calculTotauxCo2Periode($startDate, $endDate);
            $cmdTotalWeek->setCollectDate($_datetime_data);
        ...
        if ($cmdTotalMonth->getIsVisible()) {
            ...
            $totalCO2 = $this->calculTotauxCo2Periode($startDate, $endDate);
            $cmdTotalMonth->setCollectDate($_datetime_data);
    ...
    public function recordHistoryFromSuiviConso($_datas)
    { // fct appelée par l'AJAX
    ...
    public function getAndRecordDataCo2($_nbRecordsAPI = 220, $_nbRecordsATraiterDB = 14, $_eqLogic_id = NULL)
    { // fct appellée soit par l'AJAX, soit par le crouHourly
            ...
            if (isset($record['fields']['taux_co2'])) {// quand on a un noeud avec le taux_co2, on choppe les infos
                ...
                if ($datetimecherchee == $datetimerecord) {
                    ...
                    foreach (self::byType('suiviCO2', true) as $suiviCO2) {
                        ...
                        if (is_object($cmd)) {
                            $datetime_data = date('Y-m-d H:i:00', time());
                            $cmd->setCollectDate($datetime_data);
    ...
    // fb 8/5/20 Fonction exécutée automatiquement toutes les heures par Jeedom

    public static function suiviCO2hourly()
    {
        // used for debug purpose add to cron to test plugin manually
        self::cronHourly();
    }
    ...
    public static function cronHourly()
    {
        ...
        //  fb 8/5/20  decale le temps de 1 minute pour éviter le decalage des données à minuit ....
        $datetime_data = date('Y-m-d H:i:00', time() - 3600);
        $elec_present = false;

        log::add('suiviCO2', 'debug', '#################### CRON Hourly à ' . $datetime . 'pour heure ' . $datetime_data . ' ###################');
    ...
    public function postSave()
    {
        ...
        if ($this->getConfiguration('index_HC') != '') { //si on a une commande HC definie
            ...
            log::add('suiviCO2', 'info', 'Pas de commande dans le champs HC');
        }
